function [A_,Carriers_Number_,X_Carrier_Number_All_]= Eliminate_Carrier_Cant_Solved(Carriers_Number,X_Carrier_Number_All,i,A,S)
% 剔除所有解算不出来的载体的相关的信息，生成新的动作A_以及最新的应该解出来却没解出来的载体编号Carriers_Number_
    ...
    elseif E(i,1) == 0
        ...
        A_{1,2} = X;
        A_{1,3} = E;
        [flag,~,~] = Calculating_X_And_P(A_,S); % 改！
        E = Generate_Carrier_Need_Calculating(G,X);
        A_{1,3} = E;
        if flag
            ...
            [~,Carriers_Number__,X_Carrier_Number_All__] = Check_A_S(i,A_,S);
            [A_,Carriers_Number_,X_Carrier_Number_All_]= Eliminate_Carrier_Cant_Solved(Carriers_Number__,X_Carrier_Number_All__,i,A_,S);
        end
    end
end