function Z = Z_Iteration(Carrier_i,A,S,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension)
% 获取每一次迭代时的Delta_Z
    Sensor_Info = S{1,4};
    G = A{1,1};
    ...
    for j = 1:m
        if G(j,Carrier_i)==1
            ...
            if ~If_Relative
                D_ = Delta_t_Multi_C(Sender_Carrier,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension);  % 改！伪距里面还要多减去一项dt*c
                D = D + D_;
            end
            Z = [Z;D];
            temp2 = [];
