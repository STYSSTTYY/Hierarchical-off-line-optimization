function [C,Z] = Get_C_Z_i(Carrier_i,A,S)
        ...
        if (~isempty(temp3{1,i}))
            ...
            for k = 1:length(temp3)
                if (~isempty(temp3{1,k})) && (k~=i)  % 剩下的不是自己的状态估计
                    ...
                    C_ = Find_The_Position_Of_Certain_SigSquare_And_Modification(i,k,Carrier_i,C_,A,S,X,P,X_Carrier_i);  % 找出所有在载体Carrier_i上的和i有关的相对观测的sigsquare在C矩阵中的位置并且进行相应修改
                end
            end
        end
    end
