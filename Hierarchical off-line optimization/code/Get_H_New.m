function H_New = Get_H_New(Carrier_i,A,S,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension,H_)
% 获取每一次迭代时最新的H
    C = 299792458;
    ...
    for j = 1:m
        if G(j,Carrier_i)==1
            ...
            D = Get_Denominator(Sender_Carrier,If_Relative,Dimension,Object,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension);  % 改！获取每一轮迭代时对应项的分母
            if If_Relative
                if Dimension == 1
                    ...
                    h = h';
                    [Bottom,Top] = Find_The_Row_Position_Of_H(Carrier_i,j,A,S);  % 改！根据传感器编号估计其在H矩阵中的行的位置
                    for i = Bottom:1:Top
    ...
    for j = 1:n
        if A_Estimation_Trans(j,Carrier_i)==1
            ...
            [Bottom,~] = Find_The_Row_Position_Of_H_(Carrier_i,j,A,S);  % 改！根据载体编号估计其在H矩阵中的行的位置
