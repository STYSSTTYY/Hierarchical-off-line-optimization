function X_i = Gather_All_Available_Estimations_i_m(Est,Carrier,G,X,A_Array,S,S_Final,Relation_Map)
% 在X_i中记录所有载体i的可以放到载体Carrier来的状态估计
    ...
    [n,m] = size(X);
    X_Reference = Get_Estimations_Last_Step_Est(Carrier,m,X,S);  % Carrier上一步已经存在的状态估计必须纳入考虑
    X_i = zeros(n,1);
    X = X_Reference;
    Estimations_On_i = {};
    for k = 1:m
    ...
    for j = 1:n  
        ...
        if ismember(Estimation_Number,Est)  && (Relation_Map(Carrier_Number,Carrier)<3) % 如果该状态估计是Est内的状态估计且不在载体Carrier上
            if ismember(Estimation_Number,Estimations_On_i{Carrier_Number,1})  % 如果在指定载体k上确实存在该状态估计的话
                ...
                if ~Check_If_No_Unknown_Cross_Covariance(A_New,A_Array,S)
                    ...
                    if ((Cost_Com_G + S{1,3}{1,2}(Carrier_Number)) <= S_Final{1,3}{1,2}(Carrier_Number))  % 如果这可用的消息发的出来
                        X_i(j) = 1;
                    else
                        X_i(j) = 0;
                    end
        ...
        if X_Reference(j,Carrier) == 1
            X(j,Carrier) = 1;  % 假设的就是每一轮每个载体上的状态估计进入下一轮解算，所以对应的X元素必须为1
            X_i(j) = 2;
        end
    end
end