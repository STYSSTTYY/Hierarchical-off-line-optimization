function [A,S,Relation_Map_,Observation_Map_,Select_Carrier_,Flag3,Flag_Central,Flag_Partial_Concentration,RC_All] = Policy_Step_Partial_Concentration(G,X,E,State,A_Array,S_Final,Relation_Map,Observation_Map,Select_Carrier,If_Random,RC_All_)
% 策略选动作
...
% Select_Carrier 1*Num矩阵，记录载体的结果需要在哪里解算
    ...
    elseif If_Some_Satisfy_Performance_And_Costs(State,S_Final,P_All,P)  % 如果部分载体已经满足了性能要求和代价要求，已经满足了1否0
        ...
        [G,X,Flag2,Relation_Map,Observation_Map,Select_Carrier,Flag_Central,Flag_Partial_Concentration,RC_All] = Policy_1_Partial_Concentration(Temp,Estimations_Positions,A_Array,State,S_Final,G,X,Relation_Map,Observation_Map,Select_Carrier,If_Random,RC_All_);  % 有若干个性能达标时，生成一个G，X
        Case_ = 1;
        Flag3 = Flag2;
        if Flag3 == 0
            A{1,1} = NaN;
    ...
    elseif If_Cost_Meets_(State,S_Final)  % 其它，底线是必须满足Cost要求
        ...
        [G,X,Flag2,Relation_Map,Observation_Map,Select_Carrier,Flag_Central,Flag_Partial_Concentration,RC_All] = Policy_1_Partial_Concentration(Temp,Estimations_Positions,A_Array,State,S_Final,G,X,Relation_Map,Observation_Map,Select_Carrier,If_Random,RC_All_);  % 性能一个都不达标时，生成一个G，X
        Case_ = 2;
        Flag3 = Flag2;
        if Flag3 == 0
            A{1,1} = NaN;
