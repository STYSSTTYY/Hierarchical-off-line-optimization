function Com = Com_Cost_(A,S)
% 根据动作计算每一步的通讯代价，将收到的和发送的数据量分开计算，该函数专用于最后一步转移时的特殊情况的通讯量计算。
    ...
    for i = 1:Num
        for j = 1:m_s
            if (A_Sensor_Trans(j,i)==1) && (Is_Not_Own_Sensor(j,i,Sensor_Info))  % 自己的传感器不需要占用传输资源
               ...
               Cost(1,Sender_Carrier) = Cost(1,Sender_Carrier) + Data_Size(If_Relative,Dimension,Num_Of_Satellite,If_Sensor_Trans); 
               Cost(1,Receivor_Carrier) = Cost(1,Receivor_Carrier) + Data_Size(If_Relative,Dimension,Num_Of_Satellite,If_Sensor_Trans); 
            end
        end
    end
    If_Sensor_Trans = 0;
    for i = 1:Num
        for j = 1:m_e
            if (A_Estimation_Trans(j,i)==1) && (Is_Not_Own_Estimations(j,i,Sensor_Info))  % 自己储存的状态估计不需要占用传输资源
               ...
               Cost(1,Sender_Carrier) = Cost(1,Sender_Carrier) + Data_Size(0,0,Num_Of_Satellite,If_Sensor_Trans); 
               Cost(1,Receivor_Carrier) = Cost(1,Receivor_Carrier) + Data_Size(0,0,Num_Of_Satellite,If_Sensor_Trans); 
            end
        end
        for k = 1:Num  % 这边考虑协方差的传输
            ...
            Cost(1,Sender_Carrier) = Cost(1,Sender_Carrier) + Data_Size_Cross_Covariance_Matrix_(Cross_Covariance_Matrix_i);  % 计算要传输的互协方差矩阵的通信代价
            Cost(1,Receivor_Carrier) = Cost(1,Receivor_Carrier) + Data_Size_Cross_Covariance_Matrix_(Cross_Covariance_Matrix_i);
        end
    end
