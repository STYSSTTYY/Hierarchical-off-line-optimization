function [flag,X_All,P_All] = Calculating_X_And_P_(A,S,Num_Of_Satellite,Carriers_Position,Satellite_Position)
% 根据采取的动作和当前的状态进行解算 默认加权最小二乘估计迭代10轮,并且输出判断是否有解，有解flag=1,无解flag=0
    ...
    for i = 1:Num
        if E(i,1)==1
            ...
            [C,Z] = Get_C_Z_i_(i,A,S,Num_Of_Iteration,Carriers_Position,Satellite_Position);  % 改！ 根据动作和状态给出加权最小二乘估计的C，Z
            ...
            for j = 1:Num_Of_Iteration
                Delta_Z = Z - Z_Iteration_(i,A,S,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension,Num_Of_Satellite,Satellite_Position);  %改！ 获取每一次迭代时的Delta_Z
                H_New = Get_H_New_(i,A,S,X_New,X_Carrier_Number_All,Which_Has_Four_Dimension,H_,Num_Of_Satellite,Satellite_Position);  %改！ 获取每一次迭代时最新的H
                ...
                if flag3 == 0
                    flag3 = 1;
                    [Flag2_,~,~] = Check_A_S_(i,A,S,Num_Of_Satellite,Carriers_Position,Satellite_Position); % 改！
                    if Flag2_
                        flag = 0;
                        X_All = [];
                        P_All = [];
            ...
            [Flag2,~,~] = Check_A_S_(i,A,S,Num_Of_Satellite,Carriers_Position,Satellite_Position);  %改！ 根据未知数个数与方程条数的对应，找出哪些载体应该解算出来而没有解算出来，以及能不能解算出来，不能解算出来为1，能为0
            if Flag2
                flag = 0;
                X_All = [];
                P_All = [];
