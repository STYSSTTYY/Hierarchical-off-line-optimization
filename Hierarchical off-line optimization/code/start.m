function  start(Carriers,Carriers_Position,S_Final,Satellite_Position,app)
    %clc,clear;
    warning off;
    outputMessage = '';
    ...
    %     Print_Successful_Result(A_Array,S_Array,S_Final,Carriers);  % 输出成功时的结果
        ...
        Print_Successful_Result_Start(A_Array,S_Array,S_Final,Carriers,app);  % 输出成功时的结果
        Draw_Successful_Result_Start(A_Array,S_Array,S_Final,outputMatrix_G,app);  % 还没写，画出成功时的结果
        % Save_The_Results(A_Array,S_Array,FilePath);  % 将成功时的结果存入指定文件夹
    
    else  % 如果搜索失败的话
        Print_Failed_Result_Start(app);  % 输出失败时的结果
    end
end