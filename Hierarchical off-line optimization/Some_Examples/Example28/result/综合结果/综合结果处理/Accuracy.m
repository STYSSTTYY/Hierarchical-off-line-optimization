...
fig = figure('Color',[1,1,1], 'Position', [100, 100, 500, 140]);
ax = axes('Parent', fig, 'Position', [0.1, 0.1, 0.85, 0.8]); % 调整轴的位置和大小

h1 = subplot('Position', [0.08, 0.27, 0.42, 0.6]);
b = zeros(n,1);
hold on;
for i = 1:n
...
hold off;
title('The positioning accuracy histogram ','FontName', 'Times New Roman', 'FontSize', 10,'Units','pixels',Position=[211.8001392364502,89.37999999999998,0]);
ylabel('Accuracy (m^2)','FontName', 'Times New Roman', 'FontSize', 10,'Units','pixels',Position=[-18.400000324249323,48.599999999999966,0]);
set(gca, 'XTick', groupPositions(1:12, round(n/2))', 'XTickLabel', 1:1:12,'FontName', 'Times New Roman', 'FontSize', 9);
% 设置图例
legend(b, L,'FontName', 'Times New Roman', 'FontSize', 10, 'Location', 'best','NumColumns', ceil(n / 1), 'Units','pixels',Position=[39.92499785423419,-1.116279250965326,435.0000042915344,19.20000036239625]);

h2 = subplot('Position', [0.55, 0.27, 0.42, 0.6]);
b = zeros(n,1);
hold on;
for i = 1:n
...
hold off;
%xlabel('Number of the agent in the swarm','FontName', 'SimSun', 'FontSize', 10);
set(gca, 'XTick', groupPositions(13:25, round(n/2))', 'XTickLabel', 13:1:25,'FontName', 'Times New Roman', 'FontSize', 9);
...
% 计算更大的Y轴范围
ylim_combined = [min(ylim1(1), ylim2(1)), max(ylim1(2), ylim2(2))];
